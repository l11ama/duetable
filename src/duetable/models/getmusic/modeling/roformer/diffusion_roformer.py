...
from duetable.models.getmusic.utils.misc import instantiate_from_config
import numpy as np
from torch.cuda.amp import autocast
import duetable.models.getmusic.utils.midi as mc
...
class DiffusionRFM(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, log_x, t, figure_size, condition_pos, not_empty_pos, sampled=None, to_sample=None, prob=None):               # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
    ...
    def sample(
        ...
        assert batch_size == 1
        prob = None
        if params is not None and 'keep_prob' in params:
            prob = params['keep_prob']

            # replace random amount of notes in lead vocals with pad value
            replace_with_pad = torch.zeros_like(x).bool()

            replace_with_pad[:, :2, :] = torch.bernoulli(torch.full((1, 2, 256), prob)).bool()
            print(replace_with_pad.sum(-1))
            x = torch.where(replace_with_pad, torch.tensor(self.num_classes - 1), x)
            condition_pos = torch.where(replace_with_pad[0].type(torch.bool), torch.tensor(0), condition_pos)
        ...
        if params:
            num_timesteps = params.get('diffusion_steps', self.num_timesteps)
        start_step = num_timesteps

        to_be_sampled_per_step = torch.floor(num_to_be_generated_per_track / (num_timesteps - 1))
        
        self.n_sample = to_be_sampled_per_step.unsqueeze(0).repeat(num_timesteps - 1, 1)

        compensate = num_to_be_generated_per_track - self.n_sample.sum(0)

        for i in range(6):
            while compensate[i] > 0:
                compensate_freq = torch.ceil(num_timesteps / compensate[i]).long().item()
        ...
        with torch.no_grad():
            for diffusion_index in tqdm(range(start_step-1, -1, -1)):
                ...
                while (sampled < self.n_sample[diffusion_index]).any():
                    ...
                    log_p_x_on_y, sampled = self.p_sample(log_p_x_on_y, t, figure_size, condition_pos, not_empty_pos, sampled, self.n_sample[diffusion_index], prob)     # log_z is log_onehot
                    if sampled is None:
                        assert t[0] == 0
                        break
