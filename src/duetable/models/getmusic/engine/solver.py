...
from duetable.models.getmusic.utils.misc import instantiate_from_config, format_seconds
from duetable.models.getmusic.distributed.distributed import reduce_dict
from duetable.models.getmusic.distributed.distributed import is_primary
from duetable.models.getmusic.utils.misc import get_model_parameters_info
from duetable.models.getmusic.engine.ema import EMA
...
class Solver(object):
    ...
    def infer_sample(self, x, tempo, not_empty_pos, condition_pos, use_ema=True, skip_step=0, params=None):
        self.model.eval()
        tic = time.time()
        ...
        with torch.no_grad(): 
            x = x.to(self.device)
            tempo = tempo.to(self.device)
            not_empty_pos = not_empty_pos.to(self.device)
            condition_pos = condition_pos.to(self.device)
            assert x.size()[0] == 1
            ts = 6

            samples = model.infer_sample(x, tempo, not_empty_pos, condition_pos, skip_step=skip_step, params=params)
            ...
            for t in range(samples.size()[-1]):
                ...
                if (main_dur > self.pad_index):
                    print(f"dur: {main_dur}")
                if (main_pitch < self.pad_index):
                    print(f"pitch: {main_pitch}")
                ...
                if self.ids_to_tokens[root_id] in mc.root_dict and self.ids_to_tokens[kind_id] in mc.kind_dict:
                    root = mc.root_dict[self.ids_to_tokens[root_id]]
                    kind = mc.kind_dict[self.ids_to_tokens[kind_id]]
